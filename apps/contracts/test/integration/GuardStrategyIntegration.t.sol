...
contract GuardStrategyIntegrationTest is Test, IAdoptionStrategy {
    ...
    function testStrategyIntegration_AdoptionDecisions() public {
        ...
        (DataTypes.SubmissionStatus newStatus, bool shouldChange, string memory reason) =
            adoptionStrategy.evaluateSubmission(submissionId, 3, 1, 4, 1 days);
    ...
    function testStrategyIntegration_RewardCalculations() public {
        ...
        DataTypes.RewardDistribution memory distribution =
            rewardStrategy.calculateRewardDistribution(taskId, totalReward, submissionId, 2);
        ...
        uint256 accurateReward =
            rewardStrategy.calculateReviewerReward(taskId, REVIEWER1, distribution.reviewerShare, 2, true);

        uint256 inaccurateReward =
            rewardStrategy.calculateReviewerReward(taskId, REVIEWER2, distribution.reviewerShare, 2, false);
    ...
    function _reviewSubmission() internal {
        ...
        submissionManager.submitReview(
            submissionId, DataTypes.ReviewOutcome.APPROVE, "Excellent implementation, meets all requirements"
        );
    ...
    function _performReviews(uint256 approvals, uint256 rejections) internal {
        ...
        for (uint256 i = 0; i < approvals + rejections; i++) {
            ...
            DataTypes.ReviewOutcome outcome =
                i < approvals ? DataTypes.ReviewOutcome.APPROVE : DataTypes.ReviewOutcome.REJECT;
