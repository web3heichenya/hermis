...
contract GlobalGuard is BaseGuard {
    ...
    function _validateAction(address user, string memory action, uint256 reputation, DataTypes.UserStatus status)
        internal
        pure
        returns (bool success, string memory reason)
    {
        ...
        if (
            actionHash == keccak256("PUBLISH_TASK") || actionHash == keccak256("SUBMIT_WORK")
