...
contract ArbitrationManager is IArbitrationManager, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function requestUserArbitration(address user, string calldata evidence, uint256 depositAmount)
        external
        override
        nonReentrant
        returns (uint256 arbitrationId)
    {
        ...
        arbitrationId = _createArbitration(
            DataTypes.ArbitrationType.USER_REPUTATION, uint256(uint160(user)), evidence, depositAmount
        );
    ...
    function requestSubmissionArbitration(uint256 submissionId, string calldata evidence, uint256 depositAmount)
        external
        override
        nonReentrant
        returns (uint256 arbitrationId)
    {
        ...
        arbitrationId =
            _createArbitration(DataTypes.ArbitrationType.SUBMISSION_STATUS, submissionId, evidence, depositAmount);
    ...
    function resolveArbitration(uint256 arbitrationId, DataTypes.ArbitrationStatus decision, string calldata reason)
        external
        override
        onlyOwner
            ...
            if (arbitration.arbitrationType == DataTypes.ArbitrationType.USER_REPUTATION) {
                ...
                if (arbitration.depositAmount > 0) {
                    treasury.withdrawArbitrationFee(
                        arbitrationId, arbitration.requester, feeToken, arbitration.depositAmount
                    );
    ...
    function executeUserArbitration(uint256 arbitrationId, uint256 reputationIncrease)
        external
        override
        onlyOwner
        arbitrationExists(arbitrationId)
    ...
    function executeSubmissionArbitration(uint256 arbitrationId, DataTypes.SubmissionStatus newStatus)
        external
        override
        onlyOwner
        arbitrationExists(arbitrationId)
    ...
    function claimArbitrationRefund(uint256 arbitrationId)
        external
        override
        arbitrationExists(arbitrationId)
        nonReentrant
    ...
    function getArbitrationsByRequester(address requester, uint256 offset, uint256 limit)
        external
        view
        override
        returns (uint256[] memory arbitrationIds)
    {
    ...
    function getPendingArbitrations(uint256 offset, uint256 limit)
        external
        view
        override
        returns (uint256[] memory arbitrationIds)
    ...
    /// @param limit Maximum number of arbitrations to return
    /// @return arbitrationIds Array of arbitration IDs
    function getArbitrationsByType(DataTypes.ArbitrationType arbitrationType, uint256 offset, uint256 limit)
        external
        view
        override
        returns (uint256[] memory arbitrationIds)
    ...
    function canRequestArbitration(DataTypes.ArbitrationType arbitrationType, uint256 targetId)
        public
        view
        override
