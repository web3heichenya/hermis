...
contract TaskManager is ITaskManager, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createTask(
        ...
        (bool isValid,) =
            allowlistManager.validateTaskConfig(submissionGuard, reviewGuard, adoptionStrategy, rewardToken);
    ...
    function cancelTask(uint256 taskId, string calldata reason)
        external
        override
        taskExists(taskId)
        onlyTaskPublisher(taskId)
    ...
    function completeTask(uint256 taskId, uint256 adoptedSubmissionId)
        external
        override
        onlyAuthorized
        taskExists(taskId)
    ...
    function updateTaskGuards(uint256 taskId, address submissionGuard, address reviewGuard, address adoptionStrategy)
        external
        override
        taskExists(taskId)
        onlyTaskPublisher(taskId)
    {
        ...
        // Validate task configuration using AllowlistManager
        (bool isValid,) =
            allowlistManager.validateTaskConfig(submissionGuard, reviewGuard, adoptionStrategy, address(0));
        if (!isValid) revert InvalidConfiguration();
    ...
    function increaseTaskReward(uint256 taskId, uint256 additionalReward)
        external
        payable
        override
        taskExists(taskId)
        ...
        nonReentrant
    {
        ...
        if (
            task.status == DataTypes.TaskStatus.COMPLETED || task.status == DataTypes.TaskStatus.CANCELLED
                || task.status == DataTypes.TaskStatus.EXPIRED
        ) {
            revert TaskNotActive(taskId);
        }
    ...
    function getTask(uint256 taskId)
        ...
        override
        taskExists(taskId)
        returns (DataTypes.TaskInfo memory task)
    {
    ...
    function getTasksByPublisher(address publisher, uint256 offset, uint256 limit)
        external
        view
        override
    ...
    function canAcceptSubmissions(uint256 taskId)
        external
        view
        override
        taskExists(taskId)
