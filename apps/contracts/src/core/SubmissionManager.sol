...
contract SubmissionManager is ISubmissionManager, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function submitWork(uint256 taskId, string calldata contentHash)
        external
        override
        nonReentrant
    ...
    function updateSubmission(uint256 submissionId, string calldata newContentHash)
        external
        override
        submissionExists(submissionId)
        onlySubmitter(submissionId)
    ...
    function submitReview(uint256 submissionId, DataTypes.ReviewOutcome outcome, string calldata reason)
        external
        override
        submissionExists(submissionId)
        nonReentrant
    ...
    function restoreSubmissionStatus(uint256 submissionId, DataTypes.SubmissionStatus newStatus, string calldata reason)
        external
        override
        onlyAuthorized
        submissionExists(submissionId)
    ...
    function getSubmission(uint256 submissionId)
        ...
        override
        submissionExists(submissionId)
        returns (DataTypes.SubmissionInfo memory submission)
    {
    ...
    function getTaskSubmissions(uint256 taskId, uint256 offset, uint256 limit)
        external
        view
        override
        returns (uint256[] memory submissionIds)
    {
    ...
    /// @custom:view This function is read-only and supports pagination
    function getUserSubmissions(address submitter, uint256 offset, uint256 limit)
        external
        view
        override
        returns (uint256[] memory submissionIds)
    ...
    /// @custom:view This function is read-only and supports pagination
    function getSubmissionReviews(uint256 submissionId, uint256 offset, uint256 limit)
        external
        view
        override
    ...
    function getReview(uint256 reviewId)
        ...
        returns (DataTypes.ReviewInfo memory review)
    {
    ...
    /// @custom:view This function is read-only and supports pagination
    function getReviewerReviews(address reviewer, uint256 offset, uint256 limit)
        external
        view
        override
        returns (uint256[] memory reviewIds)
    ...
    function canSubmitToTask(address user, uint256 taskId)
        public
        view
        override
    ...
    /// @custom:view This function is read-only and provides detailed validation feedback
    function canReviewSubmission(address user, uint256 submissionId)
        public
        view
        override
        returns (bool canReview, string memory reason)
    {
    ...
    function _evaluateSubmissionStatus(uint256 submissionId) internal {
        ...
        (DataTypes.SubmissionStatus newStatus, bool shouldChange, string memory reason) = IAdoptionStrategy(
            ...
            submissionId, submission.approveCount, submission.rejectCount, totalReviews, timeSinceSubmission
        );
    ...
    function _processRewards(uint256 submissionId) internal {
        ...
        DataTypes.RewardDistribution memory distribution =
            rewardStrategy.calculateRewardDistribution(submission.taskId, task.reward, submissionId, reviewerCount);

        // Process creator reward
        if (distribution.creatorShare > 0) {
            treasury.withdrawTaskReward(
                submission.taskId, submission.submitter, task.rewardToken, distribution.creatorShare
            );
        ...
        if (distribution.publisherRefund > 0) {
            treasury.withdrawTaskReward(
                submission.taskId, task.publisher, task.rewardToken, distribution.publisherRefund
            );
        }
    ...
    function _processReviewerRewards(uint256 submissionId, uint256 totalReviewerReward) internal {
        ...
        for (uint256 i = 0; i < reviewCount;) {
            ...
            uint256 reviewerReward = rewardStrategy.calculateReviewerReward(
                submission.taskId, review.reviewer, totalReviewerReward, reviewCount, reviewAccuracy
            );
    ...
    function _paginateArray(uint256[] storage array, uint256 offset, uint256 limit)
        internal
        view
