...
contract HermisSBT is IHermisSBT, ERC721, Ownable {
    ...
    function updateReputation(address user, uint256 newReputation, DataTypes.UserStatus newStatus)
        external
        override
    ...
    /// @custom:security Only callable by authorized ReputationManager contract
    /// @custom:metadata Automatically refreshes token metadata after update
    function updateCategoryScore(address user, string calldata category, uint256 newScore)
        external
        override
        onlyReputationManager
    {
    ...
    function getUserCategoryScore(address user, string calldata category)
        external
        view
        override
    ...
    function getUserAllCategoryScores(address user)
        external
        view
        override
        returns (string[] memory categories, uint256[] memory scores)
    {
        uint256 tokenId = _userTokenId[user];
        if (tokenId == 0) {
    ...
    function _generateAttributes(uint256 tokenId, uint256 reputation, DataTypes.UserStatus status, uint256 stakeAmount)
        internal
        view
    ...
    function _generateSVG(uint256 tokenId) internal view returns (string memory svg) {
        ...
        string memory reputationStr =
            string(abi.encodePacked((reputation / 10).toString(), ".", (reputation % 10).toString()));
